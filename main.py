...
from dataset import *
from model import TemporalHawkesAutoformer
import logging
...
def parse_args(args=None):
    ...
    parser.add_argument('--model_name', type=str, default='AHA')
    ...
    parser.add_argument('--moving_avg', default=3, type=int)
    ...
    parser.add_argument('--AutoformerLayerNum', default=2, type=int)
    parser.add_argument('--AutoformerHeadNum', default=2, type=int)
    ...
    parser.add_argument('--forecasting_t_win_size', default=10, type=int)
...
def tes(model, testloader, skip_dict, device):
    model.eval()
    ranks = []
    logs = []
    TimeMSE = 0.
...
def main(args):
    ...
    Config = namedtuple('config', ['n_ent', 'd_model', 'n_rel','dropout','AutoformerLayerNum', 'AutoformerHeadNum'])
    config = Config(n_ent=baseDataset.num_e + 1,
                    n_rel=baseDataset.num_r * 2,
                    d_model=args.d_model,
                    dropout=args.dropout,
                    AutoformerLayerNum=args.AutoformerLayerNum,
                    AutoformerHeadNum=args.AutoformerHeadNum)
    model = TemporalHawkesAutoformer(config, args.eps, args.time_span, args.timestep, args.hmax, args.moving_avg)
    model.to(device)
        ...
        for i in range(args.max_epochs):
            if i % args.valid_epoch == 0 and i != 0:
                ...
                for delta_t in range(args.forecasting_t_win_size):
                    ...
                    metrics = tes(model, validDataLoader, baseDataset.skip_dict, device)
    ...
    if args.do_test:
        ...
        for delta_t in range(args.forecasting_t_win_size):
            ...
            metrics = tes(model, testDataLoader, baseDataset.skip_dict, device)
            for mode in metrics.keys():
