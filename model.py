...
import numpy as np
from models.GraphEncoder import NeighborAggregator, RGTEncoder, RGCNEncoder
from models.Autoformer import Autoformer, TransformerEncoder
...
class TemporalHawkesAutoformer(nn.Module):
    def __init__(self, config, eps=0.2, time_span=24, timestep=0.1, hmax=5, moving_avg=5):
        super(TemporalHawkesAutoformer, self).__init__()
        self.config = config
        self.n_ent = config.n_ent
        self.n_rel = config.n_rel
        self.d_model = config.d_model
        self.dropout_rate = config.dropout
        self.transformer_layer_num = config.AutoformerLayerNum
        self.transformer_head_num = config.AutoformerHeadNum
        ...
        self.graph_encoder = NeighborAggregator(self.d_model, self.dropout_rate)
        ...
        self.seq_encoder = Autoformer(self.d_model, self.d_model, self.transformer_layer_num,
                                              self.transformer_head_num, self.moving_avg, self.dropout_rate)
        ...
        self.linear_inten_layer = nn.Linear(self.d_model * 3, self.d_model, bias=True)
        self.time_inten_layer = nn.Linear(self.d_model * 3, self.d_model, bias=True)
    ...
    def predict_t(self, tail_ent, dur_last, query_ent_embeds, query_rel_embeds, history_gh, history_times, history_pad_mask):
        n_samples = int(self.hmax / self.timestep)  # add 1 to accomodate zero

        dur_last = torch.div(dur_last, self.time_span, rounding_mode='floor')
