...
class NeighborAggregatorLayer(nn.Module):
    def __init__(self, d_model, n_head=1, drop=0.1):
        super(NeighborAggregatorLayer, self).__init__()
        self.d_model = d_model
        self.n_head = n_head
        self.msg_fc1 = nn.Linear(self.d_model * 3, self.n_head * self.d_model, bias=True)

        self.qw = nn.Linear(self.d_model, self.n_head * self.d_model, bias=True)
        self.kw = nn.Linear(self.d_model, self.n_head * self.d_model, bias=True)
        self.temp = self.d_model ** 0.5

        self.output_afc = nn.Linear(self.n_head * self.d_model, self.d_model, bias=True)
        self.output_hfc = nn.Linear(self.d_model, self.d_model, bias=True)
    ...
    def msg_func(self, edges):
        msg = self.msg_fc1(torch.cat([edges.src['h'], edges.data['h'], edges.dst['h']], dim=-1))
    ...
    def reduce_func(self, nodes):
        ...
        h = self.dropout(F.leaky_relu(self.output_afc(h)))
        # res = self.output_hfc(res)
