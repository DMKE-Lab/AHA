...
class DGLGraphDataset(object):
    ...
    def get_nhop_subgraph(self, time, root_node, n=1):
        ...
        for i in range(n):
            ...
            for node in step_nodes:
                neighbor_n, _ = g.in_edges(node) #Dont know why is in_edge
                neighbor_n = set(neighbor_n.tolist())
                total_nodes |= neighbor_n
        sub_g = dgl.node_subgraph(g, list(total_nodes), store_ids=False)
...
class QuadruplesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.dataset_type == 'train':
            quad_idx = torch.div(idx, self.forecasting_t_windows_size, rounding_mode='floor')
    ...
    def get_history_graphs(self, head_entity, relation, timestamp, sampled_method='recent', delta_t=1):
        if sampled_method == 'history_copy':
            ...
            history_times1 = history_times1[max(-(torch.div(self.history_len, 2, rounding_mode='floor')), -len(history_times1)):]
            history_times2 = times2[:times2.index(timestamp)]
            history_times2 = history_times2[max(-(torch.div(self.history_len, 2, rounding_mode='floor')), -len(history_times2)):]
        ...
        for i in range(len(history_graphs)):
            ...
            if len(hgs) < max_history_len:
                ...
                for j in range(max_history_len - len(hgs)):
                    ...
                    g.add_edges(torch.tensor([], dtype=torch.int64), torch.tensor([], dtype=torch.int64))
                    g.edata['type'] = torch.LongTensor(np.array([]))
                    g.edata['timestamp'] = torch.ones_like(g.edata['type'])
